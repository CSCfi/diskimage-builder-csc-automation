...
function image_deploy() {
    ...
    if [[ $IMAGE_NAME =~ "GPU" ]] ; then
        ...
        This image contains Nvidia Cuda drivers" --visibility "$IMAGE_VISIBILITY" \
        "$(glance image-list | grep "$TMP_IMAGE_NAME" | cut -d '|' -f2)"
    else
        glance image-update --name "$IMAGE_NAME" --property description= \
        "All packages of this image were updated on $(date +%F)" \
        --visibility "$IMAGE_VISIBILITY" "$(glance image-list \
        | grep "$TMP_IMAGE_NAME" | cut -d '|' -f2)"
    fi
}
